...
async function showConsoleBanner() {
  ...
  const imagePath = path.join(__dirname, "src", "media", "julz-console.jpg");
  ...
  try {
    const terminalImage = await ensureModule("terminal-image");
    const figlet = await ensureModule("figlet");

    // render gambar
    if (terminalImage && fs.existsSync(imagePath)) {
      try {
        const renderer = terminalImage.default || terminalImage;
        const rendered = await renderer.file(imagePath, {
          width: Math.min(58, Math.max(28, columns - 14))
        });
        console.log(rendered);
      } catch (e) {
        console.log(chalk.red(`Gagal render gambar: ${e.message}`));
      }
    } else if (!fs.existsSync(imagePath)) {
      console.log(chalk.red(`Foto console tidak ditemukan: ${imagePath}`));
    }

    console.log("");

    // box title ala screenshot
    console.log(
      makeBox(
        `👾 ${title} ⚔️`,
        columns,
    ...
    // garis ungu
    console.log(chalk.hex("#b300ff")("━".repeat(Math.min(columns - 4, 52))));

    // info lines
    for (const row of lines) {
      if (!row.value) {
        console.log(row.l(row.label));
      } else {
        console.log(colorLine(row.label, row.value, row.l, row.v));
      }
    }
